    ...
    <main class="flex-1 pt-28 pb-20">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex items-center gap-4">
                ...
                <div>
                    <h1 class="font-outfit font-bold text-3xl text-brand-900 mb-1">Pesanan Saya</h1>
                    <p class="text-slate-500 text-sm">Lacak status pesanan Anda di sini.</p>
                </div>
            </div>

            @if(session('success'))
                <div class="bg-green-50 border border-green-200 text-green-700 px-6 py-4 rounded-2xl mb-6 text-sm font-medium flex items-center gap-3">
            ...
            @endif
            @if(session('warning'))
                <div class="bg-amber-50 border border-amber-200 text-amber-700 px-6 py-4 rounded-2xl mb-6 text-sm font-medium flex items-center gap-3">
                    <svg class="w-5 h-5 text-amber-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4.832c-.77-.833-2.694-.833-3.464 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z"></path></svg>
                    {{ session('warning') }}
                </div>
            @endif
            @if(session('error'))
                <div class="bg-red-50 border border-red-200 text-red-700 px-6 py-4 rounded-2xl mb-6 text-sm font-medium flex items-center gap-3">
